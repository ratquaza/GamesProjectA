...
public class HolySwordItem : WeaponItem
{
    ...
    public override void DefinePrimaryCollider(BoxCollider2D bc2d)
    {
        bc2d.size = new Vector2(.3f, 1f);
    }

    public override void DefineSecondaryCollider(BoxCollider2D bc2d)
    {
        bc2d.size = new Vector2(.8f, .2f);
    }
    ...
    public override void PrimaryAttack(PlayerLiving player, BoxCollider2D box)
    {
        ...
        foreach (var hit in hits)
        {
            ...
            rb2d.AddForce((enemy.transform.position - player.transform.position).normalized * 20f);
        }
    }
